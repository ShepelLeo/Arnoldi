...
use complex_iram::config::{SolverConfig, SpectrumTarget, recommended_ncv};
use complex_iram::linalg::ops::{normalize, normalized_random_vector};
...
struct Cli {
    ...
    /// Backend линейной алгебры
    #[arg(long, value_enum, default_value_t = BackendArg::Lapack)]
    backend: BackendArg,
...
fn run() -> Result<(), IramError> {
    ...
    let mut report = match cli.backend {
        BackendArg::Lapack => {
            let mut backend = LapackBackend::new();
            solve_with_backend(&mut backend, operator.as_ref(), start_vector, config, start_description)?
        }
        BackendArg::Magma => {
            #[cfg(feature = "magma")]
            {
                let mut backend = MagmaBackend::new();
                solve_with_backend(&mut backend, operator.as_ref(), start_vector, config, start_description)?
            }
            #[cfg(not(feature = "magma"))]
            {
                return Err(IramError::InvalidConfig(
                    "backend 'magma' was requested, but the binary was built without --features magma"
                        .to_string(),
                ));
            }
        }
